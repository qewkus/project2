...
from src.utils import read_json_file, filter_by_currency_csv_and_excel
...
def main():
    ...
    while True:
        ...
        if currency_filter == 'да':
            if user_input == 1:
                choosen_currency = filter_by_currency(choosen_order, "RUB")
                print(choosen_currency)
                break
            elif user_input == 2 or user_input == 3:
                choosen_currency = filter_by_currency_csv_and_excel(choosen_order, "RUB")
                print(choosen_currency)
                break
    ...
    while True:
        description_filter = input("Отфильтровать список транзакций по определённому слову в описании? Да/Нет: ").strip().lower()
